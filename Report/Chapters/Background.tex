...
\subsection{Mining Rigs}
\todo{Problem: Hard to find proper sources}
...
The power consumption on each GPU being at least 200W, making the use of rigs comparable with data centers.
...
\todo{But what about addition operations?}FPGAs provided great advantages to the speed of bitwise functions, which are important for
...
BLF was at the time the most \todo{Advice: Google and check if this still stands}successfull commercial closed-source Bitcoin company.
...
Interestingly, in contrast to other ASIC products, these were funded by the community, either through raising money over the Internet, or by preordering by the users.
\todo{Add numbers here. Or find a good spot?}
...
The Versatile Express by ARM is a family of development platforms, made for providing an environment for prototyping future \todo{Check if defined earlier}SoC designs.
With an already defined and well-proven design environment, the designer may focus on prototyping new designs, instead of creating and debugging the development framework itself. 
Versatile Express typically consists of the uATX Motherboard, on which daughterboards are supported.
In some cases, a daughterboard can allow for others to be inserted to it, the concept seen in figure \ref{fig:vexpressTiles}. 
The most common are the CoreTile Express and LogicTile express daughterboards. 
CoreTile Express contains the CPU cores that runs the system, and the version used in this project contains four Cortex A9-processors, where three of the four are disabled.
The LogicTile Express contains at least one FPGA, the number depending on the version, suitable for prototyping new logic designs. 
Of three different existing versions, our system uses the LogicTile Express 20MG (20 Million ASIC gates equivalent).
Its FPGA is a Virtex-7 XC7V2000T, and in addition, it also has A DDR3 SODIMM that supports up to 8GBs of 64bit DRAM, \todo{No idea what this meant on the data sheet}4GB SODIMM supplied.
The architectural relationship between the CoreTile Express and the LogicTile Express used in our system can be seen in figure \ref{fig:logicTile}, where both tiles are connected on the same motherboard. \cite{VexpressHomepage} \cite{LogicTileDataSheet}

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.5\textwidth]{Figures/VersatileExpress/VexpressTiles}
    \caption{Illustration of Versatile Express build, as seen from \cite{VexpressHomepage}.}
...
\begin{figure}[htb]
    ...
    \caption{Architectural relationship between CoreTile Express and LogicTile Express, as seen from \cite{LogicTileDataSheet}.}
    \label{fig:logicTile}
\end{figure}

Every reference of SHMAC in this report is the implementation that is synthesized and run on the Virtex-7 FPGA.
Although the system runs Linux version 3.14.25, Linaro 13.12, our application software is to be run by bare metal.   
  
\todo{Last sentence on the intro. Is it useful somewhere?}Versatile Express covers a range of products using processors in the ARM Cortex™ family, from Cortex-M to Cortex-A50 series of processors.
