...
\label{sec:bitcoins}
\todo{Overall: While short and concise, I feel that a lot of the facts are coming brash on. Recommendation: Make the introduction to the bitcoin system smoother for new readers. Recommend adding more of the initial history and purpose to do so.}
...
A block is only valid if the arithmetic value of the double \todo{Could probably use a little less to-the-point description regarding first mention of SHA-256}SHA-256 hash of its header is below
a certain target value. Finding a block that has such a header is \todo{Why is it computing intensive?}computing intensive, and prevents
...
refering to the fact that a \todo{Why does it take a lot? Explaination expected in SHA-256 subchapter. Make sure it is done!} lot of energy may need to be expended in order to find a valid block.
...
pools \todo{Recommend examples. The easier for the reader, the better}difficulty requirements as well as the network requirements; in that case the new block is
