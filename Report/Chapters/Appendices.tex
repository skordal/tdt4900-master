...
For each selected SHMAC architecture, the performance is measured by double hashes per second.
...
The reason for this is that all cores must make frequent accesses to external memory, which causes the
DDR-RAM tile to quickly become congested.
...
performance the individual tile has; \todo{Incorrect, round robin is used. Should affect venter tiles the more. What made you write this?}it seems that SHMACs router system favours requests originating
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Scaling using software hashing, 5x4 architecture}
	\label{fig:sw-scaling1APP}
\end{figure}
...
\begin{tabular}{| l || r r r r |}
  ...
  \hline  
\end{tabular}
\caption{Double Hashing per second, using the SHA256-hashing module. 15x2 architecture.}
\label{tab:Perf-SHA1}
\end{table}
...
\section{15x2 Tile Architecture}
...
\subsection{Software Hashing, 15x2}

The results from doing the hashing in software for this architecture is listed up in table \ref{tab:Full-Perf-SW2}. 
The CPU tiles nearest the DDR-RAM tile have highest individual performance, compared to others present.
...
\todo{Add architectural difference observation, if you see any. Double check}
...
\begin{tabular}{| l || r r r r r r r r r r r r r r|}
  ...
  \hline  
\end{tabular}
\caption{Software double hash results, this time on the 15x2 architecture}
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Scaling using software hashing, 5x4 architecture}
	\label{fig:sw-scaling2APP}
\end{figure}


\subsection{Using SHA-256 accelerator, without and with DMA, 15x2 architecture}

Tables \ref{tab:Perf-SHA2} and \ref{tab:Perf-SHADMA2} show the results when using the SHA-256 hashing accelerator in the 15x2 architecture, without and with DMA module, respectively.
...
\begin{tabular}{| l || r r r r r r r r r r r r r r |}
  ...
  \hline  
\end{tabular}
\caption{Double Hashing per second, using the SHA256-hashing accelerator. 15x2 architecture.}
\label{tab:Perf-SHA2}
\end{sidewaystable}
...
\begin{tabular}{| l || r r r r r r r r r r r r r r |}
  ...
  \hline  
\end{tabular}
\caption{Double Hashing per second, using the SHA256-hashing accelerator with the DMA module. 15x2 architecture.}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Double hashes per second using only the accelerator (red) and using both the accelerator and DMA (black). 15x2 architecture.}
...
As written in section \ref{sec:power-measure}
...
Table \ref{tab:SW-power1} shows the measured power usage, along with uncertainty and calculated application power, while \ref{fig:SW-power1} shows the plotting.
Interestingly, the more tiles are active, the less power is consumed by the system.
It appears that stalling more CPU's in waiting for memory transfer, saves power.
Most likely, before the memory congestion happens, the CPU's spend more energy in executing the hashes.

How this affects the inactive CPU's is not known.

\begin{table}
\begin{tabular}{| l | r | r || r |}
  \hline 
  \textbf{Cores} & \textbf{Average System Power} & \textbf{Uncertainty} & \textbf{Application Power} \\
  \hline                       
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Calculated power consumption using software hashing, 5x4 architecture}
	\label{fig:SW-power1}
\end{figure}


\subsection{SHA-256 hashing power consumption, without and with DMA, 5x4 Architecture.}

Tables \ref{tab:SHA-power1} and \ref{tab:SHADMA-power1} shows the individual measured power usage when using the SHA-256 accelerator without and with DMA module, respectively. 
Uncertainty and calculated application power are included as well.
Figure \ref{fig:SHA-power1} and \ref{fig:SHADMA-power1} shows the respective plottings.

As can be seen from the tables and figures, there is no noteworthy difference in power usage.
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Measured power usage in Watts, using SHA-256 accelerator, 5x4 architecture.}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Calculated power consumption using SHA-256 accelerator, 5x4 architecture}
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Measured power usage in Watts, using SHA-256 accelerator and DMA module, 5x4 architecture.}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Calculated power consumption using SHA-256 accelerator and DMA module, 5x4 architecture}
	\label{fig:SHADMA-power1}
\end{figure}

\section {15x2 Architecture}

\subsection{Software hashing power consumption, 15x2 Architecture.}

Table \ref{tab:SW-power2} and figure \ref{fig:SW-power2} shows the measured power.
Again, the trend remains the same, as less power is consumed the more tiles are active.
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Measured power usage in Watts, software only, 15x2 architecture.}
\label{tab:SW-power2}
\end{table}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Calculated power consumption using software hashing, 15x2 architecture}
	\label{fig:SW-power2}
\end{figure}

\subsection{SHA-256 accelerator power consumption, without and with DMA, 15x2 Architecture.}
\label{sec:SHA-power15x2}

Tables \ref{tab:SHA-power2} and \ref{tab:SHADMA-power2} shows the individual measured power usage when using the SHA-256 accelerator without and with DMA module, respectively. 
Uncertainty and calculated application power are included as well.
Figure \ref{fig:SHA-power2} and \ref{fig:SHADMA-power2} shows the respective plottings.

In contrast from the 5x4 architecture, this one allows all CPU-tiles to be active when using the SHA-256 accelerators.
The trend is similar to software hashing: As more tiles are active, power usage drops slightly. The drop appears to be somewhat greater when omitting any use of the DMA module.
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Measured power usage in Watts, SHA-256 accelerator, 15x2 architecture.}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Calculated power consumption using SHA-256 accelerator, 15x2 architecture}
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Measured power usage in Watts, SHA-256 accelerator with DMA, 15x2 architecture.}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Calculated power consumption using SHA-256 accelerator with DMA, 15x2 architecture}
	\label{fig:SHADMA-power2}
\end{figure}

\chapter{Calculated efficiency per Hash}

In this chapter, the efficiency will be calculated from the measured performance and energy usage.
Each architecture and configuration will be presented, where tables list up the DH/s, the power usage, and efficienty based on DH/s / W

\section{5x4 Architecture}

As a reminder, all tiles were measured when running software hashing, but only the four first tiles were measured when using the SHA-256 accelerators.

\subsection{Software hashing}

The calculation regarding the efficiency of the software hashing can be seen in table \ref{tab:SW-eff1}, and the corresponding plotting in figure \ref{fig:SW-eff1}.

It is interesting to observe that in spite of maximum performance being at 4 tiles only, with reduced performance beyond, the efficiency actually increases well beyond 4 tiles.
The drop in power usage is the main reason why. 
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Efficiency, software only, 5x4 architecture.}
\label{tab:SW-eff1}
\end{table}
...
\subsection{SHA-256 accelerator efficiency, without and with DMA}

Tables \ref{tab:SHA-eff1} and \ref{tab:SHADMA-eff1} shows the efficiency calculation of the SHA-256 accelerator, without and with using the DMA module, respectively.

The plotting for both can be seen in figure \ref{SHADMA-eff1}, where they are compared to each other.
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Efficiency, SHA-256 accelerator, 5x4 architecture.}
\label{tab:SHA-eff1}
\end{table}
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Efficiency, SHA-256 accelerator with DMA, 5x4 architecture.}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Efficiency, SHA-256 accelerator, 5x4 architecture. Black: Without DMA. Blue: With DMA}
...
In contrast to the 5x4 architecture, all tiles were used for software hashing and SHA-256 accelerator hashing.

\subsection{Software hashing, 15x2 architecture}

The efficiency, calculated from the numbers in \ref{tab:SW-eff2} and plotted in figure \ref{fig:SW-eff2}, shows the same trend as in the 5x4 architecture.
While performance is greates with only 4 tiles, the reduction in power usage makes usage of many tiles (measured 9-10 and 13-14) more efficient. 
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Efficiency, software only, 15x2 architecture.}
\label{tab:SW-eff2}
\end{table}
...
\subsection{SHA-256 hashing, without and with DMA, 15x2 architecture}

This time, with all tiles available for SHA-256 accelerator hashing, the different scaling is much more clear.
Tables \ref{tab:SHA-eff2} and \ref{tab:SHADMA-eff2} shows the calculated efficiency for the SHA-256 accelerator, without and with DMA module respectively, when scaling from 1 to all 14 CPU tiles.
They are plotted and compared in figure \ref{fig:SHADMA-eff2}.
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Efficiency, SHA-256 accelerator, 15x2 architecture.}
\label{tab:SHA-eff2}
\end{table}
...
\begin{tabular}{| l | r | r || r |}
  ...
  \hline 
\end{tabular}
\caption{Efficiency, SHA-256 accelerator with DMA, 15x2 architecture.}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Efficiency, SHA-256 accelerator, 5x12 architecture. Black: Without DMA. Blue: With DMA}
	\label{fig:SHADMA-eff2}
\end{figure}
		
		
\subsection{Comparison, efficiency on 15x2 architecture}
And at last, all 3 efficiency results are compared in figure \ref{fig:All-eff2}, and with the full scaling range of the SHA-256 accelerators, the difference between software-hashing and SHA-256 accelerator hashing becomes fully clear.
With performance scaling continually when using the accelerators, and with power usage dropping as more tiles are active, the best SHA-256 accelerator solution (no DMA) is more than 6 times more efficient than software hashing at 14 tiles.
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Efficiency, Red: Software hashing. Black: SHA-256 accelerator (no DMA). Blue: SHA-256 + DMA. 15x2 architecture}
	\label{fig:All-eff2}
\end{figure}
