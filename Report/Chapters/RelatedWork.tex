...
but consuming much less power than their GPU counterparts \cite{bespoke-silicon}.
...
However, neither GPUs nor GPUs can compare to ASICs, specialized chips that started to
appear on the network in the beginning of 2013 \cite{first-asic-miner}.

\section{The Goldstrike Bitcoin Miner}
...
The architecture, with the chosen cores and their relative sizes to one another can be seen in \todo{Figure needs resizing}
\ref{fig:Kumar1}.
...
With the total chip area anvailable for the architecture assumed to be 100 \todo{fix exponent}mm\^2, the space could consist of maximum 4 EV6 cores, or 20 EV5 cores.
