...
Any received data from a load command is sent on to the correct channel, and when a channel if finished, the imformation is passed from the Wishbone Master to the Wishbone Slave, so it can modify its registers and interrupt the CPU.\todo{Hmmm... Can agree that this is a little bit nit-picky. See if this can be summed up shorter and better.}
