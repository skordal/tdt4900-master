...
optimization specifically for bitcoin mining has been omitted; specifically, this means that the module
...
The DMA module was designed under the purpose to investigate whether separate data transfers with DMA could improve throughput and energy efficiency of the hashing process, and also gain energy savings by freeing up the on-tile CPU for other work or sleeping. A topview is seen in figure \ref{fig:DMATop}.
...
It transfers words of 32-bit data using single wishbone cycle transfers. In addition, the DMA supports
...
The wishbone slave consists of three registers for each DMA channel, used for base source address, base destination address, and detailing the transfer request.
...
The CPU tile with the integrated SHA-256 accelerator and DMA module was integrated into a grid in order
